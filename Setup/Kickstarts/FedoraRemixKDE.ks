...
%include FedoraRemixPackagesKDE.ks
...
# disable plasma-discover automatically downloading updates
if [ -f /usr/share/kf5/kdeglobals ]; then
    mkdir -p /etc/xdg
    cat >> /etc/xdg/discoverrc << 'FOE'
[Software]
UseAutoUpdate=false
FOE
fi
...
if [ -f /usr/share/applications/liveinst.desktop ]; then
  # Show harddisk install on desktop
  sed -i -e 's/NoDisplay=true/NoDisplay=false/' /usr/share/applications/liveinst.desktop
  # need to move it to anaconda.desktop
  mv /usr/share/applications/liveinst.desktop /usr/share/applications/anaconda.desktop

  # Add to Plasma favorites
  mkdir -p ~liveuser/.config
  cat > ~liveuser/.config/kickoffrc << 'FOE'
[Favorites]
FavoriteApps=preferred://browser,systemsettings.desktop,org.kde.dolphin.desktop,org.kde.konsole.desktop,anaconda.desktop
FOE
...
cat > /etc/sddm.conf.d/autologin.conf << 'FOE'
[Autologin]
User=liveuser
Session=plasma
FOE
...
# Setup KDE Plasma for liveuser
if [ -d /home/liveuser ]; then
    mkdir -p /home/liveuser/.config
    # Disable KDE welcome wizard
    touch /home/liveuser/.config/kderc
    cat > /home/liveuser/.config/kderc << 'KDERCEOF'
[KDE]
WelcomeShown=true
KDERCEOF
...
# Configure SDDM theme and login screen
mkdir -p /etc/sddm.conf.d
...
## Customize KDE Plasma Wallpaper
# Set default Plasma wallpaper (skip GNOME-specific customization)
ks_print_info "Configuring KDE Plasma wallpaper settings"
mkdir -p /etc/xdg/plasma-org.kde.plasma.desktop-appletsrc
# Wallpaper configuration for Plasma can be added here if needed
...
## Install KDE System Settings and Tools (skip GNOME-Tweaks)
ks_print_info "KDE Plasma configuration tools installed with @kde-desktop group"
