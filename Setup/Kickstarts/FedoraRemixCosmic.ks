...
%include FedoraRemixCosmicPackages.ks
...
# COSMIC Desktop - gnome-software not used
# COSMIC uses its own app store (cosmic-store)
...
if [ -f /usr/share/applications/liveinst.desktop ]; then
  # Show harddisk install in desktop
  sed -i -e 's/NoDisplay=true/NoDisplay=false/' /usr/share/applications/liveinst.desktop
  # Rename to anaconda.desktop for consistency
...
# rebuild schema cache with any overrides we installed (if any GNOME apps present)
if [ -d /usr/share/glib-2.0/schemas ]; then
  glib-compile-schemas /usr/share/glib-2.0/schemas 2>/dev/null || true
fi

# set up auto-login for COSMIC greeter
# COSMIC uses greetd with cosmic-greeter - configure autologin
mkdir -p /etc/greetd
cat > /etc/greetd/config.toml << 'FOE'
[terminal]
vt = 1
...
# Also configure GDM as fallback if present
if [ -d /etc/gdm ]; then
...
# GDM configuration - only if GDM is present (may not be with COSMIC)
...
# GDM/GNOME dconf settings - only if relevant directories exist
if [ -d /etc/dconf/db/gdm.d ]; then
...
# GNOME-specific dconf settings - only if GNOME is present
# COSMIC uses its own configuration system
if [ -d /etc/dconf/db/local.d ]; then
...
## COSMIC desktop does not use GNOME Tweaks
...
## COSMIC desktop does not use GNOME Extensions
